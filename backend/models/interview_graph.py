...
def _normalize_scores(raw_scores: dict | None) -> dict:
    ...
    for key in DEFAULT_SCORECARD:
        raw_value = (raw_scores or {}).get(key, merged[key])
        try:
            parsed[key] = float(raw_value)
        except (TypeError, ValueError):
            parsed[key] = float(merged[key])

    # 兼容旧输出：若整体看起来是 0-10 分制，则统一换算到 0-100。
    ...
    for key, value in parsed.items():
        normalized = value * 10 if scale_to_100 else value
        merged[key] = max(0, min(100, int(round(normalized))))
    return merged
...
def assessment_node(state: InterviewState):
    ...
    if user_id:
        try:
            memory_result = db_manager.ingest_user_memory_from_text(user_id=user_id, text=latest_answer, source="interview")
            if memory_result.get("updated"):
                logger.info(
                    "interview memory ingest updated: user=%s profile_updated=%s facts_added=%s",
                    user_id,
                    memory_result.get("profile_updated", False),
                    memory_result.get("facts_added", 0),
                )
        except Exception as exc:
            logger.warning("interview memory ingest failed: user=%s err=%s", user_id, exc)
...
4. scorecard 每一项必须使用 0-100 的整数分制。
"""
    try:
...
def interviewer_node(state: InterviewState):
    ...
    for message in reversed(state.get("messages", [])):
        if isinstance(message, HumanMessage) and message.content:
            memory_query = str(message.content).strip()
            break
    profile = db_manager.build_memory_context(
        user_id=user_id,
        scene="interview",
        max_facts=8,
        query=memory_query,
...
要求：
1. 直接追问，不要复述、摘抄、改写候选人刚才的回答，也不要先点评再提问。
...
def report_node(state: InterviewState):
    ...
    avg_score = round(sum(scorecard.values()) / max(len(scorecard), 1), 1)
