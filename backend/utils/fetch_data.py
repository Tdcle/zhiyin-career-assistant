...
def run_crawler():
    ...
    changed_count = inserted_total + updated_total
    if changed_count > 0:
        try:
            from utils.search_cache import bump_search_data_version

            bump_search_data_version(reason=f"manual_crawler_changed:{changed_count}")
        except Exception:
            logger.warning("manual crawler bump search data version failed", exc_info=True)

    print(
        f"\n🎉 抓取结束，共处理 {current_count} 条职位。"
        f"（新增 {inserted_total}，更新 {updated_total}）"
    )
...
def run_processor():
    ...
    if changed_total > 0:
        try:
            ...
            bump_search_data_version(reason=f"processor_changed:{changed_total}")
        except Exception:
            logger.warning("processor bump search data version failed", exc_info=True)
